...
ipcMain.handle('prompt/optimize', async (_, data) => {
    try {
        ...
        const optimizedPrompt = await huggingFaceService.optimizePrompt(data.ideaText.trim(), {
...
ipcMain.handle('comment/generate', async (_, data) => {
    try {
        ...
        // Call Hugging Face service
        const text = await huggingFaceService.generateComment(data.prompt.trim(), data.context || {});
